...
- Added `0013_ensure_default_layout.sql` to the checksum-verified PostgreSQL
  migration ledger.
- Covered parent-before-version ordering and clean-database compatibility in
  the PostgreSQL migration regression suite.
...
No application dependency or workflow changes. Cloud SQL operators apply the
repair with `make postgres-migrate-docker` through Cloud SQL Auth Proxy.
